...
<xsl:stylesheet version="1.0"
   ...
   <xsl:template match="/">
	...
	<xsl:template match="*|@*" mode="csv">
		...
		<xsl:choose>
			<xsl:when test="@code='AT-AGES'">Austria - BASG - Federal Office for Safety and Health Care (AGES - PharmMed LCM)</xsl:when>
			...
			<xsl:when test="@code='BG-NVS'">Bulgaria - Institute for Control of Veterinary Product</xsl:when>
			...
			<xsl:when test="@code='DE-BVL'">Germany - Bundesamt für Verbraucherschutz und Lebensmittelsicherheit, Ref. 301</xsl:when>
			...
			<xsl:when test="@code='ES-AGEMED'">Spain - Agencia Española de Medicamentos y Productos Sanitarios</xsl:when>
			<xsl:when test="@code='FI-NAM'">Finland - National Agency for Medicines</xsl:when>
			<xsl:when test="@code='FR-AFSSAPS'">France - AFSSAPS - Agence Française de Sécurité Sanitaire des Produits de Santé</xsl:when>
			<xsl:when test="@code='FR-ANMV'">France - ANMV - Agence Nationale du Médicament Vétérinaire, Agence Française de Sécurité Sanitaire des Aliments</xsl:when>
			<xsl:when test="@code='HU-IVMP'">Hungary - Institute for Veterinary Medicinal Products</xsl:when>
			<xsl:when test="@code='HU-OGYI'">Hungary - National Institut of Pharmacy</xsl:when>
			...
			<xsl:when test="@code='IT-LMV'">Italy - Laboratorio di Medicina Veterinaria, Istituto Superiore di Sanità</xsl:when>
			<xsl:when test="@code='IT-SPV'">Italy - Ministero della Salute, Direzione Generale della Sanità Pubblica Veterinaria</xsl:when>
			...
			<xsl:when test="@code='PT-DGV'">Portugal - DGV - Direcção Geral de Veterinária, Divisão de Meios de Defesa da Saúde Animal</xsl:when>
			...
			<xsl:when test="@code='SK-USKVBL'">Slovak Rep - Institute for State Control of Veterinary Biologicals and Medicaments Biovetská 34</xsl:when>
	...
	<xsl:template match="*|@*" mode="submission">
		<xsl:choose>
			...
			<xsl:when test="@type='referral'">Referral under Article 29, 30, 31, 35 or 36</xsl:when>
			...
			<xsl:when test="@type='transfer-ma'">Transfer of a Marketing Authorisation</xsl:when>
			...
			<xsl:when test="@type='withdrawal'">Withdrawal during Assessment or Withdrawal of a Marketing Authorisation</xsl:when>
			...
			<xsl:when test="@type='reformat'">Reformatting of an Existing eCTD Dossier (Baseline)</xsl:when>
		</xsl:choose>
	</xsl:template>
